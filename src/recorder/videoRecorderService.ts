...
export class VideoRecorderService {
  ...
  public drawBroadcastHUD(
    ...
    const driverName = (leaderCar?.state.driverName || 'LEWIS HAMILTON').toUpperCase();
  ...
  enqueueChunkExport(
    ...
    const leaderCar = instance.cars.find(c => c.state.rank === 1) || instance.cars[0];
    const winnerCar = leaderCar ? `${leaderCar.state.driverName || 'LEWIS HAMILTON'} (${leaderCar.state.name})` : 'PHANTOM APEX V12';
